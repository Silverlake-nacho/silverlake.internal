...
DEFAULT_ATLAS_SQL_QUERY = """SELECT TOP (500) *
FROM SalvageAuctions
ORDER BY DateAuction DESC;"""


def _prepare_atlas_sql_query(cursor, sql_query: str) -> str:
    """Cast SQL Server datetimeoffset columns that pyodbc cannot decode."""
    normalized_query = " ".join(sql_query.rstrip(";").split()).upper()
    normalized_default = " ".join(
        DEFAULT_ATLAS_SQL_QUERY.rstrip(";").split()
    ).upper()
    if normalized_query != normalized_default:
        return sql_query

    column_types = _fetch_table_columns(cursor, "SalvageAuctions")
    if not column_types:
        return sql_query

    select_list = ", ".join(_build_select_list(column_types))
    return (
        f"SELECT TOP (500) {select_list} "
        "FROM SalvageAuctions ORDER BY DateAuction DESC;"
    )


def fetch_atlas_sql_results(sql_query: str):
    """Run the read-only query submitted by the Atlas investigation form."""
    query = (sql_query or "").strip()
    if not re.match(r"^SELECT\b", query, flags=re.IGNORECASE):
        raise ValueError("Only SELECT queries are allowed.")

    query_without_trailing_semicolon = query[:-1] if query.endswith(";") else query
    if ";" in query_without_trailing_semicolon:
        raise ValueError("Only one SELECT query can be run at a time.")

    blocked_keywords = (
        r"\b(INTO|INSERT|UPDATE|DELETE|DROP|ALTER|TRUNCATE|MERGE|EXEC|EXECUTE|"
        r"CREATE|GRANT|REVOKE)\b"
    )
    if re.search(blocked_keywords, query, flags=re.IGNORECASE):
    ...
    for database_name in _get_atlas_db_name_candidates():
        ...
        try:
            conn = get_atlas_db_connection(database_name)
            cur = conn.cursor()
            cur.execute(_prepare_atlas_sql_query(cur, query))
            rows = cur.fetchall()
            columns = [desc[0] for desc in cur.description]
            return database_name, columns, rows
        except Exception as exc:
...
def normalize_vehicle_date_mode(mode: str) -> str:
    return "collected" if str(mode).lower() == "collected" else "recovered"


def _vehicle_date_sql_expression(date_mode: str) -> str:
    if normalize_vehicle_date_mode(date_mode) == "collected":
        return "CAST(v.CollectedDate AS datetime2)"
...
def fetch_department_sales(start_date: date, end_date: date) -> List[Tuple[str, float, float]]:
    ...
    cur.execute(
        """
        SELECT COALESCE(departmentname, 'Unknown') AS departmentname,
...
INVENTORY_ITEMNAME_EXPR = (
    "COALESCE(REPLACE(REPLACE(REPLACE(it.itemname, '[', ''), ']', ''), '_', ' '), 'Unknown')"
)


def normalize_inventory_mode(mode: str) -> str:
    return "value" if str(mode).lower() in {"value", "price", "amount"} else "count"


def normalize_inventory_order(order: str) -> str:
...
def fetch_inventory_summary(exclude_oncar: bool, with_images: bool) -> List[Tuple[str, int, float]]:
    conn = get_db_connection()
    cur = conn.cursor()
    filters = []
    params: List[object] = []
    ...
    if with_images:
        filters.append(
            ...
            EXISTS (
                SELECT 1
                FROM image img
                WHERE img.invnumber = inv.invnumber
            )
            """
        )
...
def fetch_inventory_item_monthly_totals(
    itemname: str,
    start_date: date,
    end_date: date,
    exclude_oncar: bool,
...
def fetch_inventory_item_daily_totals(
    ...
    cur.execute(
        f"""
        SELECT
            EXTRACT(DAY FROM inv.invdate)::int AS day,
            COUNT(inv.invnumber) AS item_count,
            COALESCE(SUM(COALESCE(inv.price, 0)), 0) AS total_value
        FROM inventory inv
        LEFT JOIN itemtype it ON it.itemtype_id = inv.itemtype_id
...
def shift_one_month_forward(value: date) -> date:
    """Return the same calendar day in the next month, clamped to month length."""
...
def fetch_user_monthly_totals(
    user: str, start_date: date, end_date: date
) -> List[Tuple[int, int, float]]:
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute(
...
def fetch_user_parts_monthly_totals(
    user: str, start_date: date, end_date: date
) -> List[Tuple[int, int, float]]:
    conn = get_db_connection()
    cur = conn.cursor()
...
def build_inventory_stats_context(
    mode: str,
    order: str,
    exclude_oncar: bool,
    with_images: bool,
):
    resolved_mode = normalize_inventory_mode(mode)
    resolved_order = normalize_inventory_order(order)
    rows = fetch_inventory_summary(exclude_oncar, with_images)

    def metric_value(row):
        return row[2] if resolved_mode == "value" else row[1]

    if resolved_order == "name":
        rows = sorted(rows, key=lambda row: (row[0] or ""))
    else:
        rows = sorted(rows, key=lambda row: (-metric_value(row), row[0] or ""))

    sum_count = sum(row[1] for row in rows)
    sum_value = sum(row[2] for row in rows)
    sum_total = sum_value if resolved_mode == "value" else sum_count

    chart_labels = [row[0] for row in rows]
    chart_values = [float(metric_value(row)) for row in rows]

    value_format = "currency" if resolved_mode == "value" else "count"
    value_label = "Inventory Value" if resolved_mode == "value" else "Count of Items"
    ...
    return {
        ...
        "sum_value": sum_value,
        "sum_total": sum_total,
        "inventory_mode": resolved_mode,
        "inventory_order": resolved_order,
        "exclude_oncar": exclude_oncar,
...
@app.route("/inventory_stats", methods=["GET"])
def inventory_stats():
    mode = request.args.get("mode", "count")
    order = request.args.get("order", "value")
    ...
    context = build_inventory_stats_context(mode, order, exclude_oncar, with_images)

    return render_template(
        "inventory_stats.html",
        **context,
        active_page="inventory_stats",
    )
...
def atlas_vehicle_stats():
    sql_query = request.form.get("sql_query", DEFAULT_ATLAS_SQL_QUERY).strip()
    if not sql_query:
        sql_query = DEFAULT_ATLAS_SQL_QUERY
    error_message = None
    database_name = None
    columns = []
    rows = []
    if request.method == "POST":
        try:
            database_name, columns, rows = fetch_atlas_sql_results(sql_query)
        except Exception as exc:
            error_message = f"Unable to run Atlas SQL query: {exc}"
...
def stats_department_daily(department):
    ...
    rows = fetch_rows(department, year, month)
...
@app.route("/inventory_stats/item/<path:itemname>/monthly", methods=["GET"])
def inventory_item_monthly(itemname):
    mode = normalize_inventory_mode(request.args.get("mode", "count"))
    exclude_oncar = str(request.args.get("exclude_oncar", "")).lower() in {
        "1",
        "true",
        "yes",
    ...
    end_month = shift_month(current_month_start, 1)

    rows = fetch_inventory_item_monthly_totals(
        itemname,
        start_month,
        end_month,
        exclude_oncar,
...
def inventory_item_daily(itemname):
    mode = normalize_inventory_mode(request.args.get("mode", "count"))
    exclude_oncar = str(request.args.get("exclude_oncar", "")).lower() in {
        "1",
        "true",
    ...
    rows = fetch_inventory_item_daily_totals(
        itemname,
        start_month,
        end_month,
        exclude_oncar,
    ...
    labels = []
    values = []
    for day, count_value, total_value in rows:
